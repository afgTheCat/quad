...
pub fn set_up_simulation(db: Arc<AscentDb>, simulation_id: String, logger: LogType) -> Simulator {
...
fn build_episode(db: Arc<AscentDb>, episode_name: String, training_duration: Duration) {
    ...
    let mut simulation = set_up_simulation(db.clone(), episode_name, LogType::DB);
    simulation.init();
